...
const RegisterRefugio = () => {
  ...
  const handleChange = (e) => {
    ...
    // Limitamos dirección por seguridad
    if (name === "direccion") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Error creando refugio");
      ...
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="w-full max-w-md">
          <Card className="p-8 shadow-card">
            <h2 className="text-2xl font-bold mb-6 text-center">Registro de Refugio</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white">Finalizar registro</Button>
            </form>
          </Card>
        </div>
      </div>
